...
try:
    from mooncake.buffer_pool import RegisteredBufferPool
...
except Exception:  # pragma: no cover - extension may be unavailable in pure Python tests.
...
# ---------------------------------------------------------------------------
# Remote tensor batch metadata helpers (inlined from PR2050)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# DataProto transfer helpers (migrated into structured_object_store.py)
# ---------------------------------------------------------------------------
...
def tensor_nbytes(tensor_or_shape: Any, dtype: Optional[Any] = None) -> int:
    if dtype is not None:
        return int(math.prod(tensor_or_shape)) * dtype_byte_size(dtype)
    return int(tensor_or_shape.numel() * tensor_or_shape.element_size())
...
@dataclass
class _DataProtoPoolStats:
    size_classes: Dict[int, int] = field(default_factory=dict)
    free_regions: int = 0
    in_use_regions: int = 0
    total_regions: int = 0
    total_bytes: int = 0
    free_bytes: int = 0
    acquire_count: int = 0
    ...
    oversize_allocate_count: int = 0
    wait_count: int = 0
    register_s: float = 0.0
    unregister_s: float = 0.0


class _ExternalBufferLease:
    def __init__(self, store: Any, ptr: int) -> None:
        self._store = store
        self._ptr = ptr
    ...
    def release(self) -> None:
        ...
        rc = self._store.unregister_buffer(self._ptr)
        profile_counter(self._store, "unregister_s", now() - unregister_start)
        profile_counter(self._store, "unregister_count")
        if rc != 0:
            raise RuntimeError(f"unregister_buffer failed: {rc}")
        self._released = True


class _ScratchBufferLease:
    def __init__(self, size: int) -> None:
        self.size = int(size)
        self.buffer = bytearray(self.size)
        self.ptr = ctypes.addressof(ctypes.c_char.from_buffer(self.buffer)) if self.size else 0
        self._released = False

    def __enter__(self) -> "_ScratchBufferLease":
        return self
...
class _DataProtoRegisteredBufferPool:
    def __init__(self, store: KeyValueStore) -> None:
        self.store = store
        self.max_buffer_bytes = REGISTERED_BUFFER_POOL_MAX_BUFFER_BYTES
        self._native_pool = self._create_native_pool(store)
        self._stats = _DataProtoPoolStats()

    def _create_native_pool(self, store: KeyValueStore) -> Any:
        if RegisteredBufferPool is None:
            return None
        try:
            ...
            if REGISTERED_BUFFER_POOL_PREWARM_BYTES and REGISTERED_BUFFER_POOL_PREWARM_COUNT:
                pool.prewarm(REGISTERED_BUFFER_POOL_PREWARM_BYTES, REGISTERED_BUFFER_POOL_PREWARM_COUNT)
            return pool
        except Exception:
            return None

    def try_register_external(self, ptr: int, size: int) -> Optional[_ExternalBufferLease]:
        if size == 0:
            return _ExternalBufferLease(self.store, ptr)
        register_start = now()
        rc = self.store.register_buffer(ptr, size)
        elapsed = now() - register_start
        profile_counter(self.store, "register_s", elapsed)
        profile_counter(self.store, "register_count")
        profile_counter(self.store, "registered_bytes", size)
        self._stats.register_s += elapsed
        if rc != 0:
            return None
        return _ExternalBufferLease(self.store, ptr)

    def iter_transfer_groups(self, sizes: List[int]) -> List[List[int]]:
        groups: List[List[int]] = []
        current: List[int] = []
        current_bytes = 0
        limit = max(1, int(self.max_buffer_bytes))
        for index, size in enumerate(sizes):
            size = int(size)
    ...
    def buffer(self, size: int) -> Any:
        self._stats.acquire_count += 1
        if self._native_pool is not None:
            return self._native_pool.buffer(size)
        self._stats.allocate_count += 1
        return _ScratchBufferLease(size)

    def ensure_prewarmed(self, size: int, count: int) -> None:
        if self._native_pool is not None and size > 0 and count > 0:
            self._native_pool.prewarm(size, count)

    def stats(self) -> _DataProtoPoolStats:
        return self._stats
...
def get_registered_buffer_pool(store: KeyValueStore) -> Any:
    with _STORE_STATE_LOCK:
        pool = _REGISTERED_BUFFER_POOLS.get(id(store))
        if pool is None:
            pool = _DataProtoRegisteredBufferPool(store)
