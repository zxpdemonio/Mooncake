...
std::optional<TransferFuture> TransferSubmitter::submit(
        ...
        for (size_t i = 0; i < slices.size(); ++i) {
            ...
            if (op_code == TransferRequest::READ) {
                ...
                src = reinterpret_cast<const void*>(
                    base_address + source_offset + slice_offset);
            } else {
    ...
    TransferSubmitter::submitTransferEngineOperation(
        ...
        for (size_t i = 0; i < slices.size(); ++i) {
            ...
            request.opcode = op_code;
            if (op_code == TransferRequest::READ) {
                // READ: data is copied from <target_id, target_offset> (remote)
                // to source (local) According to transfer engine docs: READ
                ...
                request.target_offset =
                    base_address + dest_offset + slice_offset;
                request.length = slice.size;
            }
            slice_offset += slice.size;
            requests.emplace_back(request);
    ...
    std::optional<TransferFuture> TransferSubmitter::submitMemoryReadOperation(
        ...
        LOG(ERROR)
            << "Read only supports LOCAL_MEMCPY or TRANSFER_ENGINE, got: "
            << strategy;
        return std::nullopt;
    }
    ...
    bool TransferSubmitter::isLocalTransfer(
        ...
        if (!local_ep.empty()) {
            std::string handle_ip =
                extractIpAddress(handle.transport_endpoint_);
    ...
    bool TransferSubmitter::validateTransferParams(
        ...
        // handle.size_ because we're reading a portion of the data. We only
        // need to check that source_offset + all_slice_len <= handle.size_
        if (op_code == TransferRequest::READ) {
            ...
            // For READ, slices can be smaller than handle.size_ when
            // source_offset > 0
            return true;
        } else {
            // For WRITE operations, slices should match handle.size_
            // (accounting for offsets) WRITE: we write from slices to handle,
            // so slices should match the writable region
